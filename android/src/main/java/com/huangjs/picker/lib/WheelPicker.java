...
public class WheelPicker extends View implements IDebug, IWheelPicker, Runnable {
   ...
   * 幕布圆角
   *
   * @see #setCurtainRound(boolean)
   */
  private boolean mCurtainRound;
  ...
  public WheelPicker(Context context, AttributeSet attrs) {
    super(context, attrs);
    mData = Arrays.asList(new String[]{});
    ...
    mMaxWidthText = "";
    mSelectedItemTextColor = -16777216;// #000
    mItemTextColor = -12303292;// #444
    mItemSpace = (int) PixelUtil.toPixelFromDIP(12);
    isCyclic = false;
    hasIndicator = true;
    mIndicatorColor = 520093696;// rgba(0,0,0,0.12)
    mIndicatorSize = (int) PixelUtil.toPixelFromDIP(1);
    hasCurtain = false;
    mCurtainColor = 520093696;// rgba(0,0,0,0.12)
    mCurtainRound = true;
    hasAtmospheric = true;
    isCurved = true;
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (hasCurtain) {
      ...
      mPaint.setStyle(Paint.Style.FILL);
      canvas.drawRoundRect(mRectCurrentItem, mCurtainRound ? 18 : 0, mCurtainRound ? 18 : 0, mPaint);
    }
    // 是否需要绘制指示器
  ...
  public boolean onTouchEvent(MotionEvent event) {
    switch (event.getAction()) {
      case MotionEvent.ACTION_DOWN:
        ...
        if (null == mTracker) mTracker = VelocityTracker.obtain();
        else mTracker.clear();
      ...
      case MotionEvent.ACTION_UP:
        ...
        // 校正坐标
        // Correct coordinates
        if (!isCyclic) if (mScroller.getFinalY() > mMaxFlingY) mScroller.setFinalY(mMaxFlingY);
        else if (mScroller.getFinalY() < mMinFlingY) mScroller.setFinalY(mMinFlingY);
        mHandler.post(this);
        if (null != mTracker) {
          mTracker.recycle();
          mTracker = null;
  ...
  private int computeDistanceToEndPoint(int remainder) {
    if (Math.abs(remainder) > mHalfItemHeight)
      if (mScrollOffsetY < 0) return -mItemHeight - remainder;
  ...
  public void setMaximumWidthTextPosition(int position) {
    if (!isPosInRang(position))
      throw new ArrayIndexOutOfBoundsException("Maximum width text Position must in [0, " + mData.size() + "), but current is " + position);
  ...
  @Override
  public boolean isCurtainRound() {
    return mCurtainRound;
  }

  @Override
  public void setCurtainRound(boolean round) {
    mCurtainRound = round;
    invalidate();
  }
